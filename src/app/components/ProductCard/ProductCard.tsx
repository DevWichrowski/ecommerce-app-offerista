...
const ProductCard = ({product}: { product: IProduct }) => {
    return (
        <Link href={`/products/${product.id}`} data-testid="product-card" className="group">
            <div className="w-full transition duration-300 ease-in-out transform hover:scale-105 hover:shadow-lg p-5">
                ...
                <h3 data-testid="product-title"
                    className="mt-4 text-sm text-gray-700 group-hover:text-gray-900 transition duration-300 ease-in-out">{product.title}</h3>
                <p data-testid="product-price"
                   className="mt-1 text-lg font-medium text-gray-900">${product.price.toFixed(2)}</p>
            </div>
        </Link>
    );
}
